...
var (
	...
	// ErrDSN is the error that returns in case of an invalid DSN.
	ErrDSN = validation.NewError("validation_is_dsn", "must be a valid DSN")
...
var (
	...
	// DSN validates if a string is a valid DSN
	DSN = validation.NewStringRuleWithError(isDSN, ErrDSN)
...
var (
	...
	// DSN regex with support for postgres and clickhouse and uri
	reDsn = regexp.MustCompile(`^(?i)(postgres|clickhouse|uri)://`)
)
...
func isDSN(value string) bool {
	return reDsn.MatchString(value)
}
